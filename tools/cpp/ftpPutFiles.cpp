 ...
 * @LastEditTime : 2025-10-17 16:02:16
...
struct arg_t
{
    ...
    char matchName[256];        // 待上传文件的匹配规则
    int pType;                  // 上传后服务器端的处理方式 : 1.不处理 2. 删除本地文件 3. 备份
    char localPathBak[256];     // 远程备份目录
    char uploadedFileList[256]; // 已上传成功文件名清单
...
map<string, string> uploadedFiles;                   // 容器一:存放已经成功上传的文件,uploadedFileList中加载
list<fileInfo_t> nlistFiles;                         // 容器二:上传前列出客户端文件名的容器,从nlist文件中加载
list<fileInfo_t> skippedFiles;                       // 容器三:本次不需要上传的文件
list<fileInfo_t> filesToUpload;                      // 容器四:本次需要上传的文件
...
void writeSkippedFiles();                            // 把容器skippedFiles中的数据写入arg,uploadedFileList文件,覆盖之前的
void appendSkippedFiles(const fileInfo_t &fileInfo); // 把上传成功的文件记录追加到arg.uploadedFileList文件中。
int main(int argc, char *argv[])
{
    // 第一步计划:从服务器某个目录中上传文件,可以指定文件名匹配的规则
    ...
    logFile.write("nlist(%s) succeed.\n", sformat("/tmp/nlist/ftpPutFiles_%d.nlist", getpid()).c_str());
    ...
    if (arg.pType == 1)
    {
        ...
        cmpContainer();
        // 把容器skippedFiles中的数据写入arg,uploadedFileList文件,覆盖之前的
        writeSkippedFiles();
    }
    else
    {
    ...
    for (const auto &file : nlistFiles)
    {
        ...
        logFile.write("put %s ... ", reFileName.c_str());
        // 调用put方法上传文件
        if (ftp.put(locFileName, reFileName, true) == false) // 上传失败
        {
            logFile.write("ftp.put(%s,%s) failed: %s.\n", reFileName.c_str(), locFileName.c_str(), ftp.response());
            return -1;
        }
        logFile.write("file(%s) proccess succeed.\n", file.fileName.c_str());
        procAct.uptatime(); // 更新进程状态
        // 上传成功后服务器的处理
        if (arg.pType == 1) // 增量上传文件
        {
            appendSkippedFiles(file);
        }
        else if (arg.pType == 2) // 删除本地文件
        {
            if (remove(locFileName.c_str()) != 0) // 删除失败
            {
                logFile.write("remove local file(%s) failed\n", locFileName.c_str());
                return -1;
            }
        }
        else if (arg.pType == 3) // 将文件移动到备份目录
        {
            string strLocFileBakPath = sformat("%s/%s", arg.localPathBak, file.fileName.c_str()); // 远程备份目录
            if (ftp.ftprename(locFileName, strLocFileBakPath) == false)                           // 移动失败
            {
                logFile.write("renamefile(%s,%s) failed.\n", locFileName.c_str(), strLocFileBakPath.c_str());
                return -1;
            }
        }
    }
...
<uploadedFileList>/tmp/idc/ftp/client/ftpGetFiles.xml</uploadedFileList>\n\
<timeout>30</timeout>\n\
<procName>ftpgetfiles_test</procName>\"\n\n\n");
    ...
    printf("<pType>1</pType> 文件上传成功后远程服务端的处理方式: 1.什么也不做; 2.删除 3.备份;如果为3还要指定备份的目录\n");
    printf("<localPathBak></localPathBak> 文件上传成功后,服务端文件的备份目录, 此参数只有当ptype为3时才生效。\n");
    printf("<uploadedFileList></uploadedFileList> 已上传成功文件名清单。此参数只有pType=1时生效\n\n\n");
}
/**
 * @brief        : 加载xml文件
...
bool xmlLoadArg(const char *xmlFile)
{
    ...
    getxmlbuffer(xmlFile, "matchName", arg.matchName, 100); // 获取待上传文件匹配的规则
    if (strlen(arg.matchName) == 0)                         // 待上传文件匹配的规则为空
    {
        logFile.write("ftp matchName is null.\n");
        return false;
    }
    getxmlbuffer(xmlFile, "pType", arg.pType); // 获取上传文件后服务器的上传方式
    if (arg.pType == 1)
    {
        getxmlbuffer(xmlFile, "uploadedFileList", arg.uploadedFileList);
        if (strlen(arg.uploadedFileList) == 0)
        {
            logFile.write("uploadedFileList bak is null\n");
            return false;
        }
    }
    else if (arg.pType == 2)
...
/**
 * @brief        : 加载已经上传的文件名到容器一uploadedFilesName中
 * @return        {*}
 **/
bool loadUpoadedFiles()
{
    uploadedFiles.clear();
    cifile inFile;
    // 如果程序第一次运行,arg.uploadedFileList是不存在的，并不是错误，所以也返回true。
    if (inFile.open(arg.uploadedFileList) == false)
    {
        return true;
    }
    string stringBuffer;
...
void cmpContainer()
{
    ...
    {
        auto it = uploadedFiles.find(nFile.fileName); // 在已上传的文件中查找nlistFiles中的文件,
        ...
        // 找到了
        if (it->second == nFile.editTime) // 时间相同不需要上传
        {
...
/**
 * @brief        : 把容器skippedFiles中的数据写入arg,uploadedFileList文件,覆盖之前的
 * @return        {*}
 **/
void writeSkippedFiles()
{
    ...
    if (outFile.open(arg.uploadedFileList) == false)
    {
        logFile.write("outFile.open(%s) failed.", arg.uploadedFileList);
    }
...
/**
 * @brief        : 把上传成功的文件记录追加到arg.uploadedFileList文件中。
...
void appendSkippedFiles(const fileInfo_t &fileInfo)
{
    ...
    if (outFile.open(arg.uploadedFileList, false, ios::app) == false)
    {
        logFile.write("outFile.open(%s) failed.\n", arg.uploadedFileList);
        return;
    }
