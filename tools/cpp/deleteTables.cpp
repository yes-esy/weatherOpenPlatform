/**
 * @FilePath     : /project/dataOpenPlatform/tools/cpp/deleteTables.cpp
 * @Description  :  删除数据库表
 * @Author       : shengYang 2900226123@qq.com
 * @Version      : 0.0.1
 * @LastEditors  : shengYang 2900226123@qq.com
 * @LastEditTime : 2025-10-21 22:33:10
...
void _help()
{
    printf("Using:/project/dataOpenPlatform/tools/bin/deletetable logfilename xmlbuffer\n\n");

    printf("Sample:/project/dataOpenPlatform/tools/bin/procctl 3600 /project/dataOpenPlatform/tools/bin/deleteTable /log/idc/deletetable_ZHOBTMIND1.log "
           "\"<connStr>idc/idcpwd</connStr><tableName>T_ZHOBTMIND1</tableName>"
           "<keyCol>rowid</keyCol>"
...
bool _xmlToRunArg(const char *strXMLBuffer)
{
    ...
    getxmlbuffer(strXMLBuffer, "tableName", runArg.tableName, 30);
    if (strlen(runArg.tableName) == 0)
    {
        logFile.write("tableName is null.\n");
        return false;
    }
...
bool _deleteTable()
{
    ...
    selectStatement.prepare("select %s from %s %s", runArg.keyCol, runArg.tableName, runArg.whereCondition);
    selectStatement.bindout(1, tmpValue, 20);
    // 1. 准备SQL语句
    string deleteSQLStr = sformat("delete from %s where %s in (", runArg.tableName, runArg.keyCol);
    ...
    {
        logFile.write("delete  from %s %d rows in %.2f sec.\n", runArg.tableName, selectStatement.rpc(), timer.elapsed());
    }
    return true;
}
/**
