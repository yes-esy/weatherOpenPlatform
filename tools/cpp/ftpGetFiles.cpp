 ...
 * @LastEditTime : 2025-10-11 18:59:05
...
struct arg_t
{
    ...
    char downloadFileName[256]; // 已下载成功文件名清单
...
map<string, string> downloadedFiles;                 // 容器一:存放已经成功下载的文件,从downloadFileName中加载
...
void writeSkippedFiles();                            // 把容器skippedFiles中的数据写入arg,downloadFileName文件,覆盖之前的
void appendSkippedFiles(const fileInfo_t &fileInfo); // 把下载成功的文件记录追加到arg.downloadFileName文件中。
int main(int argc, char *argv[])
{
    ...
    if (arg.pType == 1)
    {
        ...
        cmpContainer();
        // 把容器skippedFiles中的数据写入arg,downloadFileName文件,覆盖之前的
        writeSkippedFiles();
    }
    else
    {
...
<downloadFileName>/tmp/idc/ftp/client/ftpGetFiles.xml</downloadFileName>\n\
<timeout>30</timeout>\n\
...
<checkEditTime>true</checkEditTime>\"\n\n\n");
    ...
    printf("<downloadFileName></downloadFileName> 已下载成功文件名清单。此参数只有pType=1时生效\n");
...
bool xmlLoadArg(const char *xmlFile)
{
    ...
    if (arg.pType == 1)
    {
        getxmlbuffer(xmlFile, "downloadFileName", arg.downloadFileName);
        if (strlen(arg.downloadFileName) == 0)
        {
            logFile.write("downloadFileName bak is null\n");
            return false;
        }
...
bool loadDownloadedFiles()
{
    ...
    cifile inFile;
    // 如果程序第一次运行,arg.downloadFileName是不存在的，并不是错误，所以也返回true。
    if (inFile.open(arg.downloadFileName) == false)
    {
        return true;
    }
    string stringBuffer;
...
/**
 * @brief        : 把容器skippedFiles中的数据写入arg,downloadFileName文件,覆盖之前的
 * @return        {*}
 **/
void writeSkippedFiles()
{
    ...
    if (outFile.open(arg.downloadFileName) == false)
    {
        logFile.write("outFile.open(%s) failed.", arg.downloadFileName);
    }
...
/**
 * @brief        : 把下载成功的文件记录追加到arg.downloadFileName文件中。
...
void appendSkippedFiles(const fileInfo_t &fileInfo)
{
    ...
    if (outFile.open(arg.downloadFileName, false, ios::app) == false)
    {
